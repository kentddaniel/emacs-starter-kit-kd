es#+TITLE: Local User Customizations
...
*** Use GNU ls
OS X ships with BSD's ls command which doesn't have all the features of GNU ls (used internally by dired). 
Install `coreutils` with homebrew (`brew install coreutils`) and use `gls` instead.

*** set frame size and location; set mouse actions
#+source: display-mouse
#+begin_src emacs-lisp

  (defun workConfig ()
    (setq default-frame-alist '((top . 0) (left . 1665) (width . 126) (height . 99)))
    (setq-default fill-column 120)
    )

  (defun laptopConfig ()
    (setq default-frame-alist '((top . 2) (left . 1054)    (width . 122) (height . 82)))
    (setq-default fill-column 108)
    )

  (defun homeConfig ()
    (setq default-frame-alist '((top . 0) (left . 4220) (width . 126) (height . 99)))
    (setq-default fill-column 120)
    )

  (if (equal (getenv "MACHINE") "work") (workConfig))
  (if (equal (getenv "MACHINE") "laptop") (laptopConfig))
  (if (equal (getenv "MACHINE") "home") (homeConfig))
...
(eval-after-load "flyspell"
  ...
  ;; This prevents the async-shell-command window from coming to the front when I command-shift-click in
  ;; Skim.  I'm not sure why i need this
  ;; (see https://emacs.stackexchange.com/questions/5553/async-shell-process-buffer-always-clobbers-window-arrangement)
...
#+begin_src emacs-lisp
  ...
  ;; following added by KD 12/23/2017
...
#+source: message-line
#+begin_src emacs-lisp
      (message "Starter Kit kent.org loaded.")
#+end_src
